...
.PHONY: docs
docs: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --fm-replacements product_guard_=docs_,/products/guard/=/docs/
...
docs-skip-assets: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --skip-assets --fm-replacements product_guard_=docs_,/products/guard/=/docs/
	find ./data -name "*.json" -exec sed -i 's/https:\/\/cdn.appscode.com\/images/\/assets\/images/g' {} \;

.PHONY: assets
assets: hugo-tools
	$(HUGO_TOOLS) docs-aggregator --only-assets --fm-replacements product_guard_=docs_,/products/guard/=/docs/
	find ./data -name "*.json" -exec sed -i 's/https:\/\/cdn.appscode.com\/images/\/assets\/images/g' {} \;
	rm -rf static/files/cluster-api
	rm -rf static/files/cluster-api-provider-aws
	rm -rf static/files/cluster-api-provider-azure
