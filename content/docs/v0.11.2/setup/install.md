...
> New to Guard? Please start [here](/docs/v0.11.2/concepts).
...
If you want to set up guard via [__Kops__](https://github.com/kubernetes/kops) visit [here](/docs/v0.11.2/setup/install-kops) to see differences in setup.

If you want to set up guard via [__Kubespray__](https://github.com/kubernetes-incubator/kubespray) visit [here](/docs/v0.11.2/setup/install-kubespray).
...
Guard can use [supported authenticator](/docs/v0.11.2/guides/) to authenticate users for a Kubernetes cluster. A Kubernetes cluster can use one of these organization to authenticate users. But you can configure a single Guard server to perform authentication for multiple clusters, where each cluster uses a different auth provider.
